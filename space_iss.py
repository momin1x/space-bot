...
import time
import pycountry
hardCodedToken = 'Bearer ZTQ5NTdiNTYtZDJkYS00NDk3LTllOTctYTZiOGEzNzBkNmQ5M2U0YTljNGMtYzg3_PE93_74f575ad-38da-4d7d-aa86-0a3f07cc90cd'
# 2. Complete the if statement to ask the user for the Webex access token.
choice = input("Do you wish to use the hard-coded Webex token? (y/n) ")

if choice == 'y':
  accessToken = input("Enter your Webex token")
elif choice == 'n':
  accessToken = hardCodedToken
  print ("Thank you for your selection, the default token will be used")
else:
...
# 6. Provide the URL to the ISS Current Location API.         
        ...
        if not r.status_code ==  200:
          raise Exception( "Incorrect reply from ISS api: {}. Text: {}".format(r.status_code, r.text))
...
# # 9. Provide your Geoloaction API consumer key.
        ...
        mapsAPIGetParameters = "pk.3154b17187ae48b0b93d840608fb76bb"
        GetParameters = {
                        "lat": lat,
                        "lon": lng
                }
...
# 10. Provide the URL to the Reverse GeoCode API.
    # Get location information using the API reverse geocode service using the HTTP GET method
        r = requests.get("https://us1.locationiq.com/v1/reverse?key=pk.3154b17187ae48b0b93d840608fb76bb&format=json",
                         params = GetParameters, 
                         )
...
#     # Verify if the returned JSON data from the API service are OK
        json_data = r
        if not r.status_code ==  200:
          raise Exception( "Incorrect reply from geo location api {}. Text: {}".format(r.status_code, r.text))
...
# 11. Store the location received from the API in a required variables
        ...
        CountryResult = address.get("country", "Unknown")
        CountryCodeResult = address.get("country_code", "Unknown")
        StateResult = address.get("state", "Unknown")
        CityResult = address.get("city", address.get("town", address.get("village", "Unknown")))
        StreetResult = address.get("road", "Unknown")
        
        #Find the country name using ISO3611 country code
        if not CountryResult == "XZ":
            CountryResult = pycountry.countries.get(alpha_2=CountryCodeResult.upper())
...
# 13. Complete the code to post the message to the Webex room.         
        ...
        if not r.status_code ==  200:
          raise Exception( "Incorrect reply from Webex: {}. Text: {}".format(r.status_code, r.text))